...
class SettingController extends Controller
{
    ...
    // manual wallet
    public function manualWallet(Request $request)
    {

        // check if the payment processor is set to manual
        if (site('payment_processor') !== 'manual') {
            return response()->json(validationError('Set your payment processor to manual first'), 422);
        }
        ...
        //check if all 10 values are filled
        $wallets  = $request->wallet_address;
        if (count($wallets) < 0) {
            return response()->json(validationError('At least 1 field is required'), 422);
        }

        // check for regex
        $coin = DepositCoin::find($request->coin_id);
        $wallet_regex = $coin->wallet_regex; //^(0x)[0-9A-Fa-f]{40}$
        $wallet_addresses = $request->wallet_address;

        foreach ($wallet_addresses as $address) {
            if ($address && $address !== 'null') {
                if (!preg_match('/' . $wallet_regex . '/', $address)) {
                    return response()->json(validationError("The wallet [$address] you submitted is not valid for your selected coin"), 422);
                }
            }
        }
        ...
        // get the coin
        $coin = DepositCoin::find($request->coin_id);
        $coin->wallet_address = json_encode($wallets);
        $coin->save();

        return response()->json(['message' => $coin->name . ' wallet address updated successfully']);
    }
}
